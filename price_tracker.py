"""
Nykaa Price Tracker - Bright Data Web Scraper API
----------------------------------------------------
Track prices, availability, and product data from Nykaa.

Docs  : https://docs.brightdata.co.kr/scraping-automation/web-scraper-api/overview
Info  : https://brightdata.co.kr/products/insights/price-tracker/nykaa
"""
...
# Set your Web Scraper ID after building your Nykaa scraper:
# https://brightdata.co.kr/products/web-scraper/nykaa
DATASET_ID = os.getenv("BRIGHTDATA_DATASET_ID", "")
if not DATASET_ID:
    raise EnvironmentError(
        "BRIGHTDATA_DATASET_ID is not set.\n"
        "1. Build your Nykaa scraper at: https://brightdata.co.kr/products/web-scraper/nykaa\n"
...
def track_prices(urls: list[str], **kwargs) -> list[dict]:
    """
    Collect price data for a list of Nykaa product URLs.
...
def discover_by_keyword(keyword: str, limit: int = 50, **kwargs) -> list[dict]:
    """
    Discover Nykaa products by keyword search.
...
def discover_by_category(category_url: str, limit: int = 100, **kwargs) -> list[dict]:
    """
    Discover all products from a Nykaa category page.

    Example:
        results = discover_by_category("https://nykaa.com/s?k=headphones", limit=200)
    """
...
if __name__ == "__main__":
    ...
    sample_urls = [
        "https://www.nykaa.com/product/sample-item-123456",
        # Add more Nykaa product URLs here
    ]
    results = track_prices(sample_urls)

    output_file = "nykaa_prices.json"
    ...
    # cat_results = discover_by_category("https://nykaa.com/category/sample", limit=50)
